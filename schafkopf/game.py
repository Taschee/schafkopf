...
class Trick:
    def __init__(self, playerlist, leading_player):
        ...
        self.winner = None
        self.num_cards = 0
        self.leading_playerindex = leading_player
    ...
    def determine_trickwinner(self, trumpcards):
        ...
        if len(trumps_in_trick) > 0:
            ...
            starting_index = self.leading_playerindex
...
class Game:
    def __init__(self, players, leading_player=0, cards=[(i % 8, i // 8) for i in range(32)]):
        ...
        self._tricks = []
        self._current_trick = {"cards": [None for pl in self._playerlist], "num_cards": 0}
        self._current_player_index = leading_player
        self._leading_player_index = leading_player
    ...
    def possible_cards(self, current_trick, hand):
        if current_trick["num_cards"] == 0:
            ...
            first_card = current_trick["cards"][current_trick["leading_playerindex"]]
    ...
    def determine_trickwinner(self, trick):
        # returns index of winning card / player
        trumps_in_trick = [card for card in trick["cards"] if card in self._trump_cards]
        if len(trumps_in_trick) > 0:
            best_trump = min([self._trump_cards.index(trump) for trump in trumps_in_trick])
            trick["score"] = calculate_points(trick)
            trick["winner"] = trick["cards"].index(self._trump_cards[best_trump])
            return trick["winner"]
        else:
            starting_index = trick["leading_playerindex"]
            first_card = trick["cards"][starting_index]
            played_suit = first_card[1]
            best_card = (max([i for (i, j) in trick["cards"] if j == played_suit]), played_suit)
            trick["score"] = calculate_points(trick)
            trick["winner"] = trick["cards"].index(best_card)
            return trick["winner"]
    ...
    def play_next_card(self):
        ...
        if self._current_trick["num_cards"] == 0:
            self._current_trick["leading_playerindex"] = self._current_player_index
        options = self.possible_cards(self._current_trick, current_player.get_hand())
        next_card = current_player.play_card(previous_cards=self._current_trick, options=options)
        ...
        if self._current_trick["num_cards"] == 4:
            self.determine_trickwinner(self._current_trick)
